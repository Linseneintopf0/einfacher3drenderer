...
//Konvertiert message IDs zu derren Text mit der Hash-Map unten

std::string Outputlog::msgIdConv(UINT msg)
{
	auto output = Outputlog::msglist.find(msg);
	if (output != Outputlog::msglist.end())
	{
...
//Mit ChatGPT generiert; ich hab keine Ahnung wie das funktioniert lol

void Outputlog::AttachConsoleWindow() {
	AllocConsole(); // Create a console window
	FILE* fDummy;
	freopen_s(&fDummy, "CONOUT$", "w", stdout); // Redirect stdout to the console
	std::cout.clear(); // Clear any error flags
}

//List mit allen message Codes, die bekannt sind -> https://stackoverflow.com/questions/7252398/convert-windows-message-ids-to-text

std::unordered_map<unsigned int, std::string> Outputlog::msglist = {
  { 0, "WM_NULL"},
  { 1, "WM_CREATE" },
  { 2, "WM_DESTROY" },
  { 3, "WM_MOVE" },
